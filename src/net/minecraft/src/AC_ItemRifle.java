...
public class AC_ItemRifle extends Item {
	private static final int MAX_AMMO = 30;

	public AC_ItemRifle(int itemId) {
		super(itemId);
		this.maxStackSize = 1;
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		if(!world.multiplayerWorld) {
			if(stack.getItemDamage() < MAX_AMMO) {
				...
				arrow.setArrowHeading(arrow.motionX, arrow.motionY, arrow.motionZ, 4.0F, 0.5F);
				world.entityJoinedWorld(arrow);
				world.playSoundAtEntity(player, "random.explode", 0.4F, 2.5F);
